...
class A1_NeuralNetwork(object):
    ...
    def train(self, numlayer = 1, numnode = 100, rate = 0.6, num_epoches = 20, momentum = 0.9, batchsize = 10): #train the neural network with datasets preloaded into memory
        ...
        ## initialize weight with pretrained weights
        np.random.seed()
            
        self.weights = [np.zeros((1,1))] 
           

        data = list(csv.reader(open('../weights.csv', 'rU')))
        wei = ((6**0.5)/(19**0.5))*np.random.randn(100,784)
        for i in xrange(784):
            for j in xrange(100):
                wei[j][i] = float(data[i][j])
   
        self.weights = self.weights + [wei]

        for r, c in zip(self.arch[2:], self.arch[1:-1]):
